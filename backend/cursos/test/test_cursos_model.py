...
class Curso:
    def __init__(self, cur_id=1, usu_id=None, tcu_id=None, per_id_responsable=None, car_id_responsable=None, com_id_lugar=None, cur_fecha_hora=None, cur_fecha_solicitud=None, cur_codigo="C001", cur_descripcion="Curso de Prueba", cur_observacion="Ninguna", cur_administra=1, cur_cuota_con_almuerzo=100.0, cur_cuota_sin_almuerzo=80.0, cur_modalidad=1, cur_tipo_curso=1, cur_lugar="Sala 1", cur_estado=1):
        self.cur_id = cur_id
        self.usu_id = usu_id
        self.tcu_id = tcu_id
...
class CursoAlimentacion:
    def __init__(self, cua_id=1, cur_id=None, ali_id=None, cua_fecha=None, cua_tiempo=2, cua_descripcion="Almuerzo Estándar", cua_cantidad_adicional=0, cua_vigente=True):
        self.cua_id = cua_id
        self.cur_id = cur_id
        self.ali_id = ali_id
...
class PersonaCurso:
    def __init__(self, pec_id=1, per_id=None, cus_id=None, rol_id=None, ali_id=None, niv_id=None, pec_observacion="Ninguna", pec_registro=True, pec_acreditado=False):
        self.pec_id = pec_id
        self.per_id = per_id
        self.cus_id = cus_id
        self.rol_id = rol_id
...
def test_cursoalimentacion_creation():
    ...
    assert curso_alimentacion.cua_vigente == False
...
def test_cursoformador_creation():
    ...
    assert curso_formador.cuo_director == True
...
def test_personacurso_creation():
    ...
    assert persona_curso.pec_registro == True
    assert persona_curso.pec_acreditado == True
...
def test_personaestadocurso_creation():
    ...
    assert persona_estado_curso.peu_vigente == True
    assert str(persona_estado_curso) == f"Estado {persona_estado_curso.peu_estado} para {mock_persona_curso} por {mock_usuario}"

# Note: In a real Django project, you would import the actual models like:
# from ..models import Curso, CursoSeccion, CursoFecha, CursoCuota, CursoAlimentacion, CursoCoordinador, CursoFormador, PersonaCurso, PersonaEstadoCurso
