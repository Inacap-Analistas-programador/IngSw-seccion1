...
class PersonaCurso(models.Model):
    id = models.DecimalField(
        db_column="pec_id", max_digits=10, decimal_places=0, primary_key=True
    )
    ...
    nivel = models.ForeignKey(
        Nivel, on_delete=models.RESTRICT, db_column="niv_id", blank=True, null=True
    )
    ...
    envio_correo_qr = models.BooleanField(db_column="pec_envio_correo_qr")
...
class PersonaEstadoCurso(models.Model):
    id = models.DecimalField(
        db_column="peu_id", max_digits=10, decimal_places=0, primary_key=True
    )
...
class PersonaVehiculo(models.Model):
    id = models.DecimalField(
        db_column="pev_id", max_digits=10, decimal_places=0, primary_key=True
    )
    persona_curso = models.ForeignKey(
        PersonaCurso, on_delete=models.RESTRICT, db_column="pec_id"
    )
