...
class CourseViewSet(viewsets.ModelViewSet):
    ...
    queryset = Course.objects.select_related("category", "created_by").prefetch_related(
        "team__user"
    )
    permission_classes = [permissions.AllowAny]  # Temporalmente público
    ...
    def dashboard_metrics(self, request):
        ...
        warning_courses = Course.objects.filter(
            status=Course.ACTIVE, end_date__gte=today, end_date__lte=warning_date
        ).count()
        overdue_courses = Course.objects.filter(
            status=Course.ACTIVE, end_date__lt=today
        ).count()
