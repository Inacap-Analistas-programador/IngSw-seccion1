...
class PagoPersona(models.Model):
    ...
    USU_ID = models.ForeignKey(User, on_delete=models.PROTECT, related_name='pagos_realizados', help_text="Usuario que registra la transacción.")
    PAP_FECHA_HORA = models.DateTimeField(default=timezone.now, help_text="Fecha y hora exactas en que se registra el pago.")
    ...
    PAP_VALOR = models.DecimalField(max_digits=10, decimal_places=2, help_text="Monto del pago. Debe ser un valor positivo.")
    PAP_OBSERVACION = models.CharField(max_length=255, blank=True, null=True, help_text="Notas adicionales sobre el pago (ej. 'Abono 50%').")
...
class PagoCambioPersona(models.Model):
    ...
    """
    PCP_ID = models.AutoField(primary_key=True, help_text="Identificador único del registro de cambio.")
    PER_ID = models.IntegerField(help_text="ID de la nueva persona a la que se le asigna el pago. FK al modelo de Personas.")
    PAP_ID = models.ForeignKey(PagoPersona, on_delete=models.PROTECT, related_name='cambios_pago', help_text="Pago original que se está transfiriendo.")
    USU_ID = models.ForeignKey(User, on_delete=models.PROTECT, related_name='cambios_pago_realizados', help_text="Usuario que autorizó y registró el cambio.")
    PCP_FECHA_HORA = models.DateTimeField(default=timezone.now, help_text="Fecha y hora en que se realizó el cambio.")
...
class Prepago(models.Model):
    ...
    """
    PPA_ID = models.AutoField(primary_key=True, help_text="Identificador único del prepago.")
    PER_ID = models.IntegerField(help_text="ID de la persona a la que pertenece el saldo. FK al modelo de Personas.")
    CUR_ID = models.IntegerField(help_text="ID del curso donde se puede usar este saldo. FK al modelo de Cursos.")
    PAP_ID = models.ForeignKey(PagoPersona, on_delete=models.SET_NULL, null=True, blank=True, related_name='prepago_asociado', help_text="Pago original que generó este saldo a favor (opcional).")
    PPA_VALOR = models.DecimalField(max_digits=10, decimal_places=2, help_text="Monto del saldo a favor.")
    PPA_OBSERVACION = models.TextField(blank=True, null=True, help_text="Notas sobre el origen o condición del prepago.")
    PPA_VIGENTE = models.BooleanField(default=True, help_text="Indica si el saldo a favor todavía está disponible para ser usado.")
...
class ConceptoContable(models.Model):
    ...
    - 'Venta de Uniforme'
    """
    COC_ID = models.AutoField(primary_key=True, help_text="Identificador único del concepto contable.")
    COC_DESCRIPCION = models.CharField(max_length=100, unique=True, help_text="Nombre descriptivo del concepto (ej. 'Inscripción Anual').")
    COC_VIGENTE = models.BooleanField(default=True, help_text="Indica si el concepto está activo y puede ser utilizado en nuevas transacciones.")
...
class ComprobantePago(models.Model):
    ...
    """
    CPA_ID = models.AutoField(primary_key=True, help_text="Identificador único del comprobante.")
    USU_ID = models.ForeignKey(User, on_delete=models.PROTECT, related_name='comprobantes_creados', help_text="Usuario que emitió el comprobante.")
    PEC_ID = models.IntegerField(help_text="ID de la Persona-Curso a la que se emite el comprobante. FK a PersonaCurso.")
    COC_ID = models.ForeignKey(ConceptoContable, on_delete=models.PROTECT, related_name='comprobantes_concepto', help_text="Concepto contable principal del comprobante.")
    CPA_FECHA_HORA = models.DateTimeField(default=timezone.now, help_text="Fecha y hora de emisión del comprobante.")
    CPA_FECHA = models.DateField(default=timezone.localdate, help_text="Fecha del comprobante (sin hora) para reportes.")
    CPA_NUMERO = models.IntegerField(unique=True, help_text="Número correlativo y único del comprobante.")
    CPA_VALOR = models.DecimalField(max_digits=12, decimal_places=2, help_text="Valor total del comprobante, puede ser la suma de varios pagos.")
...
class PagoComprobante(models.Model):
    ...
    """
    PCO_ID = models.AutoField(primary_key=True, help_text="Identificador único de la relación.")
    PAP_ID = models.ForeignKey(PagoPersona, on_delete=models.CASCADE, related_name='pagos_comprobante', help_text="Pago que forma parte del comprobante.")
    CPA_ID = models.ForeignKey(ComprobantePago, on_delete=models.CASCADE, related_name='comprobantes_pago', help_text="Comprobante que agrupa el pago.")
