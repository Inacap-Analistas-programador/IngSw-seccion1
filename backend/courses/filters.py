...
class CursoFilter(filters.FilterSet):
    ...
    rama = filters.CharFilter(
        field_name="secciones__rama__nombre", lookup_expr="icontains"
    )
    fecha_desde = filters.DateFilter(
        field_name="fechas__fecha_inicio", lookup_expr="gte"
    )
    fecha_hasta = filters.DateFilter(
        field_name="fechas__fecha_termino", lookup_expr="lte"
    )
