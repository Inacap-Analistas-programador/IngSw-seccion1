...
def check_environment():
    ...
    if not Path("manage.py").exists():
        print_colored(
            "ERROR: No se encuentra manage.py. Ejecuta desde el directorio backend/",
            Colors.RED,
        )
        return False
...
def run_tests(args):
    ...
    # Ejecutar tests
    success = run_command(
        base_command, f"Ejecutando tests {'con cobertura' if args.coverage else ''}"
    )
...
def main():
    ...
    # Opciones generales
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Salida verbose de los tests"
    )
    parser.add_argument(
        "-c",
        "--coverage",
    ...
    parser.add_argument(
        "--file", metavar="FILE_PATH", help="Ejecutar tests de un archivo específico"
    )
    ...
    parser.add_argument(
        "-m", "--migrations", action="store_true", help="Verificar migraciones"
    )

    # Opción todo-en-uno
    parser.add_argument(
        "--all", action="store_true", help="Ejecutar tests, linting y verificaciones"
    )
