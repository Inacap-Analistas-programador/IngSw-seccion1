...
const MaestroForm = () => {
  ...
  useEffect(() => {
    ...
    const personasNoFormadores = storedPersonas.filter(persona => 
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content={`${isEdit ? 'Editar' : 'Crear'} formador en el sistema Scout.`} />
      </Helmet>
          ...
          <form onSubmit={handleSubmit} className="max-w-4xl mx-auto">
            ...
            >
                  ...
                  >
                    ...
                    {personas.map(persona => (
                      <option key={persona.id} value={persona.id}>
                        {persona.nombres} {persona.apellidoPaterno} {persona.apellidoMaterno} - {persona.rut}-{persona.dv}
                      </option>
                    ))}
                  </select>
                  {errors.personaId && <p className="text-red-500 text-sm mt-1">{errors.personaId}</p>}
                </div>
              )}
              ...
              {selectedPersona && (
                ...
                >
                  <h3 className="font-semibold text-scout-azul-oscuro mb-3">Información de la Persona:</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p><strong>Profesión:</strong> {selectedPersona.profesion || 'No especificada'}</p>
                      <p><strong>Apodo:</strong> {selectedPersona.apodo}</p>
                    </div>
                  </div>
                </motion.div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-2">
                  <div className="bg-gray-50 rounded-lg p-6">
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">Estado del Formador</h3>
                    <div className="flex items-center mb-4">
                      <EstadoIcon className={`w-8 h-8 mr-3 ${
                        estadoFormador.color === 'green' ? 'text-green-500' :
                        estadoFormador.color === 'yellow' ? 'text-yellow-500' :
                        'text-red-500'
                      }`} />
                      <div>
                        <p className={`text-lg font-semibold ${
                          estadoFormador.color === 'green' ? 'text-green-700' :
                          estadoFormador.color === 'yellow' ? 'text-yellow-700' :
                          'text-red-700'
                        }`}>
                          {estadoFormador.estado}
                        </p>
                        <p className="text-sm text-gray-600">
                          {formData.habilitacion1 && formData.habilitacion2 && formData.verificacion
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between text-sm">
                        ...
                        <span>{[formData.habilitacion1, formData.habilitacion2].filter(Boolean).length}/2</span>
                      </div>
