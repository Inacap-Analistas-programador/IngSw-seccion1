...
const Pagos = () => {
  ...
  const [form, setForm] = useState({ per_id: '', cur_id: '', pap_tipo: 1, pap_valor: '', pap_observacion: '' });
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => setShowForm(true)} className="bg-primary text-primary-foreground hover:opacity-90">
          Registrar Pago
        </Button>
      </div>
      ...
      <div className="bg-card rounded-lg shadow-md p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <label className="block">
            ...
            <input type="text" placeholder="ID o Nombre de Persona" className="mt-1 w-full rounded-md p-2 bg-input text-card-foreground" />
          </label>
          <label className="block">
            <div className="text-sm text-muted-foreground">Filtrar por Curso</div>
            <input type="text" placeholder="ID o Nombre de Curso" className="mt-1 w-full rounded-md p-2 bg-input text-card-foreground" />
          </label>
          <label className="block">
            <div className="text-sm text-muted-foreground">Filtrar por Fecha</div>
            <input type="date" className="mt-1 w-full rounded-md p-2 bg-input text-card-foreground" />
          </label>
        </div>
        <Button className="mt-4 bg-secondary text-secondary-foreground hover:opacity-90">Aplicar Filtros</Button>
      </div>

      <div className="bg-card rounded-lg shadow-md p-6">
        {loading && <p className="text-muted-foreground">Cargando pagos...</p>}
        {error && <p className="text-destructive-foreground">Error: {String(error.message || error)}</p>}

        {!loading && rows.length === 0 && <p className="text-muted-foreground">No hay pagos registrados.</p>}
                ...
                {rows.map((p) => (
                  <tr key={p.pap_id} className="odd:bg-background/60">
                    ...
                    <td className="py-2 px-3 flex gap-2">
                      ...
                      <button onClick={() => onDelete(p.pap_id)} className="text-destructive-foreground underline">
                        Eliminar
                      </button>
                    </td>
                  </tr>
      ...
      <div className="bg-card rounded-lg shadow-md p-6">
        ...
        <p className="text-muted-foreground">Aquí se gestionarán los comprobantes de pago, se emitirán nuevos y se verá el historial.</p>
        <div className="flex gap-2 mt-4">
          <Button className="bg-primary text-primary-foreground hover:opacity-90">Emitir Comprobante</Button>
          <Button className="bg-secondary text-secondary-foreground hover:opacity-90">Ver Historial de Comprobantes</Button>
        </div>
      </div>
      ...
      <div className="bg-card rounded-lg shadow-md p-6">
        ...
        <p className="text-muted-foreground">Aquí se mostrará un registro de todos los cambios realizados en los pagos.</p>
        <Button className="mt-4 bg-secondary text-secondary-foreground hover:opacity-90">Ver Historial Completo</Button>
      </div>
      ...
      <div className="bg-card rounded-lg shadow-md p-6">
        ...
        <Button className="mt-4 bg-primary text-primary-foreground hover:opacity-90">Registrar Prepago</Button>
      </div>

      {/* Sección de Dashboard de Pagos */}
      <div className="bg-card rounded-lg shadow-md p-6">
        <h2 className="text-xl font-semibold mb-4">Dashboard de Pagos</h2>
        <p className="text-muted-foreground">Resumen visual de totales, pagos pendientes y pagos por curso.</p>
        <Button className="mt-4 bg-primary text-primary-foreground hover:opacity-90">Ver Dashboard</Button>
      </div>
      ...
      {showForm && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          <form onSubmit={onSubmit} className="relative bg-card text-card-foreground rounded-lg p-6 w-full max-w-md shadow-lg">
            <h2 className="text-lg font-semibold mb-4">Registrar Pago</h2>
            <div className="space-y-3">
              <label className="block">
                <div className="text-sm text-muted-foreground">Persona ID</div>
                <input name="per_id" value={form.per_id} onChange={onChange} className="mt-1 w-full rounded-md p-2 bg-input text-card-foreground" />
              </label>
              <label className="block">
                <div className="text-sm text-muted-foreground">Curso ID</div>
                <input name="cur_id" value={form.cur_id} onChange={onChange} className="mt-1 w-full rounded-md p-2 bg-input text-card-foreground" />
              </label>
              <label className="block">
                <div className="text-sm text-muted-foreground">Tipo</div>
                <select name="pap_tipo" value={form.pap_tipo} onChange={onChange} className="mt-1 w-full rounded-md p-2 bg-input text-card-foreground">
              ...
              <label className="block">
                ...
                <input name="pap_valor" value={form.pap_valor} onChange={onChange} type="number" step="0.01" className="mt-1 w-full rounded-md p-2 bg-input text-card-foreground" />
              </label>
              <label className="block">
                <div className="text-sm text-muted-foreground">Observación</div>
                <input name="pap_observacion" value={form.pap_observacion} onChange={onChange} className="mt-1 w-full rounded-md p-2 bg-input text-card-foreground" />
              </label>
            </div>

            <div className="flex justify-end gap-2 mt-4">
              <Button type="button" onClick={() => setShowForm(false)} className="bg-muted text-muted-foreground">Cancelar</Button>
              <Button type="submit" className="bg-primary text-primary-foreground">Guardar</Button>
            </div>
          </form>
        </div>
      )}
